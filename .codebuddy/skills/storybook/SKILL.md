---
name: storybook
description: 儿童绘本生成器。根据故事主题生成完整的儿童绘本：角色设计 → 分镜脚本 → 逐页生成 16:9 高清插图。通过角色参考图保持跨页面视觉一致性。使用 codebuddy 内置的文生图模型 gemini-3.0-pro-image 生成图片。
---
...
**拼接网格图命令**（所有页面图片生成完成后自动执行）：
```bash
${BUN_X} {baseDir}/scripts/stitch-grid.js <book-dir> --no-open
```
...
用户输入 → Step 1 (编排器: 分析主题 + 流程规划，读 what-is-storybook.md)
  ...
  → Step 8 (🤖 Sub-Agent: 逐页生成内页图片) → *.png
  → Step 8.5 (编排器: 拼接网格图，不查看结果) → all-pages.png
  → Step 9 (编排器: 完成报告)
```
...
技术要求：严格按模板格式，每页含故事文字（中英双语）、画面描述（镜头/构图/角色动作/环境）、情绪标注、翻页钩子。镜头类型要变化，情绪弧线清晰。
...
**封面（00-cover）— 无前一页，只带角色参考图：**
  使用默认的文生图模型，生成封面文件 {outputDir}/00-cover.png
  要求：
...
**后续每页（01-page 起）— 带角色参考图 + 前一页图片：**
  ...
  使用默认的文生图模型，生成文件： {outputDir}/{NN}-page.png
  要求：
  1 prompt：{outputDir}/prompts/{NN}-page.md
  2 参考角色参考图: {outputDir}/characters/characters.jpeg
  3 参考前一页： {outputDir}/{前一页文件名}.png \
  4 尺寸 16:9 的 2K 图片

生成顺序: 00-cover, 01-page, 02-page, ...

### 关键要求
- 每一页都必须传入 --ref characters/characters.jpg
- 从 01-page 开始，额外传入前一页的 .png 作为第二个 --ref
- 封面（00-cover）只带角色参考图，不带前一页（因为没有前一页）
- 01-page 的前一页是 00-cover.png
...
### Step 8.5: 拼接网格图（编排器直接执行）

所有内页图片生成完毕后，自动执行拼接脚本，将封面和所有内页拼接为一张网格图：

```bash
${BUN_X} {baseDir}/scripts/stitch-grid.ts {outputDir} --no-open
```

生成文件: `{outputDir}/all-pages.png`

⚠️ **不要阅读或展示 `all-pages.png`**。该文件通常 >5MB、分辨率极高（如 2752x16896px），用 Read 工具加载会非常慢且浪费大量 token。用户可以自行在 Finder 中打开查看。只需在 Step 9 完成报告中列出文件路径即可。
...
文件列表:
  ...
  ✓ 00-cover.png             - 封面
  ✓ 01-page.png ~ NN-page.png - 内页（共 N 页）
  ✓ all-pages.png             - 全页网格图

用户可在 Finder 中打开输出目录查看所有图片，或直接打开 all-pages.png 预览完整绘本。

如需对某页图片做细节调整（边距、文字、表情等），使用：
/refine storybook/{slug}-{timestamp}/03-page.png 你的修改指令
```
...
- **⚠️ 读取图片前必须先压缩**：生成的原图通常有数 MB，直接用 Read 工具阅读会导致错误。**任何时候需要阅读/展示 .png 图片，必须先用 `sips -Z 1024 <原图> --out /tmp/<预览名>.png` 压缩到 1024px 以内，再 Read 压缩后的文件**。这包括：角色参考图确认、内页生成结果检查、用户要求查看某页等所有场景。
- 图片生成每页约 10-30 秒，请耐心等待
- 失败自动重试一次
- **角色参考图必须经用户确认后才能开始生成内页**（Step 7 的确认环节）
- 每一页都必须带 `--ref` 角色参考图以保持角色一致性
- 从第 01-page 起，额外传入前一页的 .png 作为第二个 `--ref`，保持场景风格延续
