...
用户输入 → Step 1 (编排器: 分析主题 + 流程规划，读 what-is-storybook.md)
  ...
  → Step 8.5 (编排器: 拼接网格图，不查看结果) → all-pages.jpeg
  → Step 9 (编排器: 完成报告)
```
...
- 场景自然文字（招牌、书本标题等）：必须在 prompt 中明确写出具体的中文+英文两种文字内容
